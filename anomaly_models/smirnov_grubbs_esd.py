...
Modified to include anomaly scores\rank - Dipanjan
...
class GrubbsTest(object):
    ...
    def _test_once(self, data, alpha, get_score=True):
        ...
        score = (g - g_test)
        if get_score: 
            return (target_index, score) if g > g_test else (None, None)
        else:
            return target_index if g > g_test else None

    def run(self, alpha=DEFAULT_ALPHA, output_type=OutputType.DATA, get_score=True):
        ...
        while True:
            if get_score:
                outlier_index, score = self._test_once(data, alpha, get_score=get_score)
                if score:
                    scores.append(score)
            else:
                outlier_index = self._test_once(data, alpha, get_score=get_score)
...
def _test(test_class, data, alpha, output_type, get_score=True):
    return test_class(data).run(alpha, output_type=output_type, get_score=get_score)


def _two_sided_test(data, alpha, output_type, get_score=True):
    return _test(TwoSidedGrubbsTest, data, alpha, output_type, get_score)


def _min_test(data, alpha, output_type, get_score=True):
    return _test(MinValueGrubbsTest, data, alpha, output_type, get_score)


def _max_test(data, alpha, output_type, get_score=True):
    return _test(MaxValueGrubbsTest, data, alpha, output_type, get_score)


def two_sided_test(data, alpha=DEFAULT_ALPHA, get_score=True):
    return _two_sided_test(data, alpha, OutputType.DATA, get_score)


def two_sided_test_indices(data, alpha=DEFAULT_ALPHA, get_score=True):
    return _two_sided_test(data, alpha, OutputType.INDICES, get_score)


def two_sided_test_outliers(data, alpha=DEFAULT_ALPHA, get_score=True):
    return _two_sided_test(data, alpha, OutputType.OUTLIERS, get_score)


def min_test(data, alpha=DEFAULT_ALPHA, get_score=True):
    return _min_test(data, alpha, OutputType.DATA, get_score)


def min_test_indices(data, alpha=DEFAULT_ALPHA, get_score=True):
    return _min_test(data, alpha, OutputType.INDICES, get_score)


def min_test_outliers(data, alpha=DEFAULT_ALPHA, get_score=True):
    return _min_test(data, alpha, OutputType.OUTLIERS, get_score)


def max_test(data, alpha=DEFAULT_ALPHA, get_score=True):
    return _max_test(data, alpha, OutputType.DATA, get_score)


def max_test_indices(data, alpha=DEFAULT_ALPHA, get_score=True):
    return _max_test(data, alpha, OutputType.INDICES, get_score)


def max_test_outliers(data, alpha=DEFAULT_ALPHA, get_score=True):
    return _max_test(data, alpha, OutputType.OUTLIERS, get_score)


def test(data, alpha=DEFAULT_ALPHA, get_score=True):
    return two_sided_test(data, alpha, get_score)