...
from CONFIG import nwalarm_event_config as conf
import pandas as pd
...
for idx, row in event_types_df.iterrows():
    ...
    if ts_obj.concept_drift_present:
        ...
        kibana_params = {'loc': evt_type['loc']}
        nwalarm_loc_grps[key] = {'event_type' : evt_details}
        nwalarm_loc_grps[key]['raw_ts'] = ts
        if not anomaly_df.empty:
            ...
            nwalarm_loc_grps[key]['anomalies'] = anomaly_df
            
        
    
    

all_anomalies = [pd.concat([pd.DataFrame([nwalarm_loc_grps[key].get('event_type')])\
                                                            .reset_index(drop=True),
                            nwalarm_loc_grps[key].get('anomalies')\
                                              .reset_index(drop=True)
                            ],
                           #ignore_index=True, 
                           axis=1)
                for key in nwalarm_loc_grps.keys() 
                    if 'anomalies' in nwalarm_loc_grps[key].keys()]   
...
all_anomalies_df = all_anomalies_df[all_anomalies_df.score > 0.01]
